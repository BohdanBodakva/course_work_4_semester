...
const char* serverName = "http://192.168.1.104:8080/api/users/bestuser/devices/device1/post-esp32-values";
...
int currentMillis = 0;
int currentIrrigationMillis = 0;
volatile int frequencyMillis = 3000;

volatile float expectedSoilMoisture = 2048;
...
void handleRequest(AsyncWebServerRequest *request) {
    ...
    request->send(200, "text/plain", "Parameters were set successfully");
}
...
void loop() {
  ...
  if(millis() - currentIrrigationMillis > 3000){
    ...
    currentIrrigationMillis = millis();
  }
